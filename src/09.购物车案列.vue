  ...
  <el-container>
    ...
    <el-main>
      <tr v-for="item in list" :key="item.id">
        ...
        <td>￥{{ (item.price * (item.corunt || 1)).toFixed(2) }}</td>
    ...
    <el-footer>
      ...
      <div class="right-footer" style="display: flex">
        <span>已经选{{ totalList }}件商品</span>
        <span style="margin: 0 20px">商品总价：￥{{ total.toFixed(2) }}</span>
...
<script>
import img1 from './assets/QQ截图20220702100209.png'
import img2 from './assets/QQ截图20220702100234.png'
import img3 from './assets/QQ截图20220702100251.png'
export default {
  data() {
    return {
      list: [
  ...
  computed: {
    ...
    total() {
      return this.list.reduce((sum, next) => {
        return next.c ? sum + (next.price * next.corunt) : 0
      }, 0)
    },
    //商品数量
    totalList() {
      return this.list.reduce((sum, next) => {
        return next.c ? sum + next.corunt : sum
      }, 0)
    },
  },
}
